...
export default function EditTransactionClient({ params }: EditTransactionClientProps) {
  ...
  const submitTransaction = async () => {
    ...
    try {
      ...
      addToast(
        err instanceof Error ? err.message : 'Ocorreu um erro ao atualizar o lançamento.',
        'error',
      );
    } finally {
      setSubmitting(false);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setDeleteError(
        err instanceof Error ? err.message : 'Não foi possível excluir este lançamento.',
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const deleteTargetLabel =
    reference.trim() || `${tipo === 'income' ? 'Receita' : 'Custo'} de R$ ${amount || '0,00'}`;
        ...
        <button
          ...
          title={
            transaction?.status === 'confirmed'
              ? 'Lançamentos confirmados não podem ser excluídos'
              : 'Excluir lançamento'
          }
        >
          <Trash2 size={20} />
        ...
        <div className="flex-1 p-6 md:p-10 max-w-3xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-y-auto pb-48 md:pb-32">
          <div className="flex flex-col gap-8">
            ...
            <div className="flex bg-gray-100/80 p-1 rounded-xl h-[48px] opacity-70 cursor-not-allowed">
              <div
                className={`flex-1 flex items-center justify-center text-sm font-semibold rounded-lg ${tipo === 'income' ? 'bg-white text-emerald-600 shadow-sm border border-gray-200/50' : 'text-gray-400'}`}
              >
                Receita
              </div>
              <div
                className={`flex-1 flex items-center justify-center text-sm font-semibold rounded-lg ${tipo === 'expense' ? 'bg-white text-red-600 shadow-sm border border-gray-200/50' : 'text-gray-400'}`}
              >
                Custo
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium text-gray-700">
                Pedido vinculado <span className="text-gray-400 font-normal">(opcional)</span>
              </label>
              <div className="relative">
                <select
                  value={orderId}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl animate-in zoom-in-95 duration-200">
            <div className="flex flex-col gap-4">
              ...
              <div className="text-center">
                <h3 className="text-base font-bold text-gray-900">Excluir lançamento?</h3>
