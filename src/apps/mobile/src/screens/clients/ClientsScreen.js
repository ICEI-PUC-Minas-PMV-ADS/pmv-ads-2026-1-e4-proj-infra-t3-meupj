...
export default function ClientsScreen() {
	...
	useFocusEffect(
		useCallback(() => {
			const timer = setTimeout(() => fetchClients(tabRef.current, searchRef.current), 300);
	...
	const handleSearchChange = (text) => {
		setSearch(text);
		const timer = setTimeout(() => fetchClients(tabRef.current, text), 300);
		return () => clearTimeout(timer);
	};

	const handleTabChange = (key) => {
		setActiveTab(key);
		fetchClients(key, searchRef.current);
	};
		...
		return (
			...
			>
				...
				<View style={styles.cardInfo}>
					...
					<Text style={styles.cardContact} numberOfLines={1}>{getContactInfo(item)}</Text>
				</View>
